...
### 1. Фикс "Segmentation Fault"
Проблема падения ядра на Linux решена использованием «золотого стека» версий PaddlePaddle 2.6.2 + PaddleOCR 2.8.1. Это гарантирует 100% стабильность на CPU.

### 2. Точность координат в 4K (Ultra HD)
Для попадания в эталон разметки при обработке в 1080p используется формула трансформации:
X_global = X_local * Scale
Y_global = (Y_local + ROI_offset) * Scale

### 3. Интеллектуальный парсинг
Регулярные выражения извлекают price_default и price_card до финальной очистки текста, что позволяет сохранять копейки и корректно вычислять скидки.
